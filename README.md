...
- `active_span: OpenTracing::Span` an OpenTracing span, optional. A span that will be used as parent for all span that will be generated by Redis. If no active span is supplied, all span from Redis will be a root span.

This library supports all tracer implementation that is compatible with OpenTracing.
If you already create a span and set it as `active_span`, that span will be used as parent span for all span created by Redis.
