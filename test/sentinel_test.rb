...
class SentinelTest < Test::Unit::TestCase
  ...
  def test_sentinel_connection_with_specified_master
    sentinels = [{:host => "127.0.0.1", :port => 26381},
                 {:host => "127.0.0.1", :port => 26382}]
    ...
    RedisMock.start(handler.call(:s1)) do |s1_port|
      RedisMock.start(handler.call(:s2)) do |s2_port|
        sentinels[0][:port] = s1_port
        sentinels[1][:port] = s2_port
        redis = Redis.new(:url => "redis://master1", :sentinels => sentinels, :master_name => 'master2', :role => :master)

        assert redis.ping
      end
    end

    assert_equal commands[:s1], [%w[get-master-addr-by-name master2]]
    assert_equal commands[:s2], []
  end
