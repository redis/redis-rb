...
describe "redis" do
  ...
  it "should properly marshall objects" do
   ...
   @r.set('fail', MyFail.new, true)
   @r.get('fail', true).fail.should == 'it will'
  ...
  it "should be able to MGET keys" do
    @r['foo'] = 1000
    @r['bar'] = 2000
    @r.mget(false, 'foo', 'bar').should == ['1000', '2000']
    @r.mget(false, 'foo', 'bar', 'baz').should == ['1000', '2000', nil]
  end
  
  it "should quit" do
    lambda {@r.quit}.should raise_error(RedisError)
  end
