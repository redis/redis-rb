...
`redis.gemspec` declares one native extension, `ext/redis/xxh3/extconf.rb`, providing `Redis::XXH3.hexdigest` (`lib/redis/xxh3.rb`) — local XXH3-64 digest computation for `SET`'s `IFDEQ`/`IFDNE` and `DELEX`'s `IFDEQ`/`IFDNE` options (Redis 8.4+ CAS/CAD), so a `match-digest` can be built without a round trip to `DIGEST`. It is **off by default**: `extconf.rb` checks `enable_config("xxh3", false)` and writes a no-op `dummy_makefile` unless the caller passes `--enable-xxh3` (`gem install redis -- --enable-xxh3`, or `bundle config build.redis --enable-xxh3` before `bundle install`) — the same `dummy_makefile` fallback technique `hiredis-client`'s own `extconf.rb` uses for platforms it doesn't build on. This keeps the gem's build 100% unaffected (no compiler needed, no behavior change) for the overwhelming majority of callers who never touch local digest computation, while still shipping the extension in the same package rather than a separate gem. `ext/redis/xxh3/{xxhash.h,xxhash.c}` vendor the exact xxHash v0.8.3 (BSD-2-Clause) source the Redis server itself uses (`deps/xxhash` in the server tree), so a locally computed digest and one fetched via `DIGEST` are byte-identical by construction, not just "the same algorithm." Build it locally in this checkout with `rake compile` (root `Rakefile`); `test/redis/xxh3_test.rb` and `test/redis/digest_consistency_test.rb` both skip themselves if that hasn't been run. `redis.gemspec`'s `s.files` is restricted to `lib/**/*.rb` and `ext/**/*.{c,h,rb}` (not a blanket glob) specifically so a local `rake compile` artifact never gets swept into a built `.gem` package.
